...
public class StudentAdapter extends RecyclerView.Adapter<StudentAdapter.StudentViewHolder> implements Filterable {
    ...
    public StudentAdapter(Context context, Cursor cursor) {
        ...
        while(mCursor.moveToNext()){
            ...
            String pic = mCursor.getString(mCursor.getColumnIndex(DataBaseContract.StudentEntry.COLUMN_PIC));

            mlist.add(new StudentItem(id,name,email,status,pic));
        }
        if(!mlist.isEmpty()){
    ...
    public void onBindViewHolder(@NonNull StudentViewHolder holder, final int position) {
        ...
        holder.picview.setText(item.StudentPic);
...
class StudentItem{
    ...
    StudentItem(int id,String name,String email,String status,String pic){
        Studentid=id;
        StudentName=name;
        StudentEmail=email;
